...
class SmbopParser(Model):
    def __init__(
        ...
        self._evaluate_func = partial(
            evaluate_single,
            db_dir='',
            table_file=os.path.join("dataset", "squall_tables.json"),
        )
    ...
    def _create_type_tensor(self):
        ...
        for rule in rules:
            ...
            try:
                ...
                continue
            rule_tensor[i][j][k] = 1
        #print(self.value_pred)
        #print(len(self._type_dict))
        #print(len(rule_tensor))
...
a boolean vector to tell if a given span is a gold span (i.e it corrosponds to a span_hash that is in hash_gold_levelorder[0]).
    ...
    def forward(
        ...
        #print(final_leaf_schema_scores)
        ...
        #print('min_k', min_k, self._n_schema_leafs)
        ...
        for decoding_step in range(self._decoder_timesteps):
            ...
            '''
            for eee in new_hash_gold_levelorder[decoding_step + 1][0]:
                if eee not in beam_hash[0] and eee.item() != -1:
                    print('wrong!!!', eee)
                    print('step===========', decoding_step)
                    print(node_util.print_tree(tree_obj[0]))
                    print(beam_hash)
                    print(frontier_hash)


                    print(is_levelorder_list, is_levelorder_list.size())
                    print(torch.sum(is_levelorder_list[0]))
                    print(new_hash_gold_levelorder[decoding_step + 1][0], new_hash_gold_levelorder[decoding_step + 1].size())
                    print(new_hash_gold_levelorder)
                    print(outputs["leaf_beam_hash"])
                    input()
            '''
            #print(node_util.print_tree(tree_obj[0]))
            #input()
        ...
        if not self.training:
            ...
            for b in range(batch_size):
                ...
                try:
                    ...
                    #print(node_util.print_tree(tree_res))
