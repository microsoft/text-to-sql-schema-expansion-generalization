...
def get_literals(tree):
    ...
    for node in findall(tree, filter_=lambda x: x.name == "Value"):
        if type(node.val) == str and  node.val != '' and node.val[:2] == 'w.':
            continue
        if type(node.val) == str and  node.val == 'ID':
            continue
        value_list.append(str(node.val))
    return value_list
